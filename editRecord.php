...
<?php
   ...
   $dbConnection = DatabaseConnection::connect();
    ...
    if (empty($_POST)){
        ...
        $name_first = odbc_result($res, 'Name_first');
        $name_last = odbc_result($res, 'Name_last');
        $name_MI = odbc_result($res, 'Name_MI');
        $Referrer = odbc_result($res, 'Referrer');
        $Employer = odbc_result($res, 'Employer');
        $Date_of_birth_input = odbc_result($res, 'Date_of_birth');
        $date = new DateTime($Date_of_birth_input);
        $Date_of_birth = $date->format('m-d-y');
        $Address_street = odbc_result($res, 'Address_street');
        $Address_city = odbc_result($res, 'Address_city');
        $Address_state = odbc_result($res, 'Address_state');
        $Address_zip = odbc_result($res, 'Address_zip');
        $Cell_phone = odbc_result($res, 'Cell_phone');
        $Home_phone = odbc_result($res, 'Home_phone');
        $agent_ID = odbc_result($res, 'agent_ID');               
                
    }
    else {

        //gathers input from the user
        $hiddenID = filter_input(INPUT_POST, 'cust_ID');
...
<html>
    ...
    <body>
        ...
        <br>updating (i.e. SQL UPDATEs) of data in your CUSTOMERS table in your Access database.<p>
        ...
        <form  action="editRecord.php" method="post">
            <input type="hidden" value="<?php echo !empty($hiddenID)?$hiddenID:$hiddenID;?>" name="cust_ID">
        <table>
                <tr>
